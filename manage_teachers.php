...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action'])) {
    ...
    if ($action == 'add') {
        ...
        $stmt = $conn->prepare("INSERT INTO users (name, email, password, role, department, status) VALUES (?, ?, ?, 'teacher', ?, 'approved')");
        $stmt->bind_param("ssss", $name, $email, $password, $dept);
        if($stmt->execute()) $msg = "Teacher appointment successful.";
        ...
        if($stmt->execute()) $msg = "Faculty record updated.";
        else $msg = "Error: " . $conn->error;
    }
}

// Handle GET (Delete/Toggle)
if (isset($_GET['action']) && isset($_GET['id'])) {
    ...
    if ($_GET['action'] == 'delete') {
        ...
        $new = ($current == 'approved') ? 'rejected' : 'approved';
...
<head>
    ...
    <title>Faculty Management | ExamSystem Pro</title>
...
<body>
    <div class="admin-layout">
        <aside class="admin-sidebar">
            ...
            <nav>
                ...
                <a href="manage_teachers.php" class="sidebar-link active"><i class="fas fa-chalkboard-teacher"></i> Faculty</a>
        ...
        <main class="admin-main">
            <header style="margin-bottom: 40px; display: flex; justify-content: space-between; align-items: center;">
                <div>
                    <h1 style="font-size: 2rem; font-weight: 800; color: var(--secondary);">Faculty Roster</h1>
                    <p style="color: var(--text-muted);">Manage academic staff and departmental assignments</p>
                </div>
                <button class="btn-nav-primary" onclick="openModal('add')"><i class="fas fa-plus"></i> Appoint Faculty</button>
            </header>

            <div class="card animate-fade-in">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px;">
                    <div class="input-with-icon" style="flex: 1; max-width: 400px;">
                        <input type="text" id="searchInput" placeholder="Search faculty by name..." onkeyup="searchTable()">
                        <i class="fas fa-search"></i>
                    </div>
                    <span class="badge badge-blue">Total Faculty: <?php echo $result->num_rows; ?></span>
                </div>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td style="padding: 20px 24px;">
                                    <div style="font-weight: 700; color: var(--secondary);">Prof. <?php echo htmlspecialchars($row['name']); ?></div>
                                    <div style="font-size: 0.85rem; color: var(--text-muted);"><?php echo htmlspecialchars($row['email']); ?></div>
                                </td>
                                <td>
                                    <span class="badge badge-blue"><?php echo strtoupper($row['department'] ?? 'GENERAL'); ?></span>
                                </td>
                                <td>
                                    <span class="badge <?php echo $row['status'] == 'approved' ? 'badge-success' : 'badge-error'; ?>">
                                        <?php echo strtoupper($row['status'] ?? 'PENDING'); ?>
                                    </span>
                                </td>
                                <td>
                                    ...
                                    <a href="?action=toggle&id=<?php echo $row['id']; ?>" class="btn-nav-primary" style="padding: 6px 12px; font-size: 0.8rem; background: #f59e0b;" onclick="return confirm('Toggle faculty access?')"><i class="fas fa-power-off"></i></a>
                                    <button class="btn-nav-primary" style="padding: 6px 12px; font-size: 0.8rem; background: #ef4444;" onclick="confirmPurge(<?php echo $row['id']; ?>)"><i class="fas fa-trash-alt"></i></button>
                                </td>
                            </tr>
                            <?php endwhile; ?>
                        </tbody>
    ...
    <!-- Enhanced Modal -->
    <div id="teacherModal" class="modal" style="display:none; position:fixed; z-index:1000; left:0; top:0; width:100%; height:100%; background:rgba(0,0,0,0.5); align-items:center; justify-content:center;">
        <div class="card" style="width:100%; max-width:500px; margin:20px;">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:24px;">
                <h2 id="modalTitle" style="font-weight:800;">Appoint Faculty</h2>
            ...
            <form method="post">
                ...
                <div class="form-group">
                    ...
                    <div class="input-with-icon">
                        <input type="text" name="name" id="teacherName" required placeholder="Preceded by Prof. / Dr.">
                ...
                <div class="form-group">
                    ...
                    <div class="input-with-icon">
                        <input type="email" name="email" id="teacherEmail" required placeholder="faculty@institution.edu">
                ...
                <div class="form-group">
                    ...
                    <div class="input-with-icon">
                        <input type="text" name="department" id="teacherDept" placeholder="e.g. Computer Science">
                        <i class="fas fa-building-columns"></i>
                    </div>
                </div>

                <div style="display:flex; gap:16px; margin-top:32px;">
                    <button type="submit" class="btn-block">Commit Appointment</button>
    ...
    <script>
        function openModal(mode, data = null) {
            ...
            if (mode === 'add') {
                document.getElementById('modalTitle').textContent = 'Appoint Faculty';
                ...
                document.getElementById('teacherDept').value = data.department;
            }
        }
        ...
        function confirmPurge(id) {
            Swal.fire({
                ...
                text: "The faculty member will lose all access to the cluster and exam banks.",
                icon: 'warning',
